...
You can reduce noise and improve correlation by building your own custom decisions. To start building a decision, go to **[one.newrelic.com](https://one.newrelic.com/all-capabilities) > Alerts & AI** > **Decisions**, then click **Create new decision**.

We offer two versions of the decision builder:

* Basic decision builder: This version covers the majority of use cases and focuses on "correlate by attribute," where you can specify filter conditions for correlation matches. You can also apply the same filter logic for specific values to both incidents being correlated. For example, you can correlate incidents if the entity name is `host 1` for both.
* Advanced decision builder: In addition to the features the basic decision builder provides, this one allows for more complex decision creation by applying different logic filters to the two incidents being correlated. For example, you can correlate incidents if one has entity name `host 1` and the other has entity name `host 2`.
...
**The basic decision builder is currently in preview for some customers.**
...
1. Choose an attribute from the dropdown menu.
2. The `equal` operator, often the most common choice, is preselected. You can also choose another [operator](#operators).
3. The second attribute usually matches the first, so it's autopopulated. You can keep the autopopulated option or choose another operator.
4. Click `Add` to include this filter. A [simulation](#simulations) will run automatically.
5. Repeat steps 1-4 to add up to eight logic filters.

#### Step 2: Filter by specific values [#basic-filter-values]

1. To open the `Filter by specific values` section for additional filters, click `See more options`.
2. Choose an attribute from the dropdown menu.
...
5. Click `Add` to include this filter. [Simulation](#simulations) will run automatically.
6. Repeat steps 2-5 to add up to eight logic filters.

#### Step 3: Filter by related entities [#basic-filter-related-entities]

1. Click `Filter by related entities`.
2. Choose entity classes and save.

For automatic topology correlation, make sure your telemetry data is collected by [New Relic agents](/docs/new-relic-solutions/new-relic-one/install-configure/compatibility-requirements-new-relic-agents-products/). Learn more about our out-of-the-box [topology correlation](#topology-requirements).

In addition, you can set up topology data via our NerdGraph `aiTopologyCollector`. (Search for `aiTopology` in the [NerdGraph GraphiQL explorer](/docs/apis/nerdgraph/examples/topology-nerdgraph-tutorial)). This allows any topology-related decision to be matched with your topology data. Learn more about [setting up topology correlation](#topology).

#### Step 4: Set correlation time range [#basic-set-time-range]

The correlation time range is set to 20 minutes by default. You can adjust the range between 1 minute and 120 minutes to suit your specific needs.

#### Step 5: Testing your rule with simulation [#basic-test-with-simulation]

1. After adding a filter logic, the system automatically runs a [simulation](#simulations) using the past 7 days of incident data.
2. You can also manually trigger simulation by clicking `Simulate` if anything changes in the rule.

#### Step 6: Name and save your decision [#basic-name-save-decision]

1. Click `Create decision` to access the name and description panel.
2. The system generates a name based on your rule. Customize the name and description as desired.
3. Click `Save decision` to finalize and save your new decision in the system.

### Advanced decision builder [#advanced-decision-builder]

For more complex rule creation, click `Use advanced builder` to create custom rules for your specific requirements. 
...
* **Incident examples:** A list of incident pairs that the decision would have correlated, including the rule's attributes and values, as well as other popular attributes in each pair. Click on incidents to view details.
